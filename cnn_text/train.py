...
tf.flags.DEFINE_string("data_file", "./training.csv", "Data source for the  data.")
# tf.flags.DEFINE_integer("num_labels", 2, "Number of labels for data. (default: 2)")
tf.flags.DEFINE_integer("num_labels", 11, "Number of labels for data. (default: 2)")
...
tf.flags.DEFINE_float("l2_reg_lambda", 0.09, "L2 regularization lambda (default: 0.0)")
...
# Load data
print("Loading data...")
x_text, y = data_helpers.load_data_files(FLAGS.data_file)
print(y)
...
with tf.Graph().as_default():
    ...
    """
    4773个样本，每个样本有570个字，每个字128维
    x.shape = (4773, 570, 128)
    y.shape = (4773, 11)
    """
	...
	# Define Training procedure
        ...
        def train_step(x_batch, y_batch):
            ...
            _, step, summaries, loss, entro_loss, l2_loss, accuracy = sess.run(
                [train_op, global_step, train_summary_op, cnn.loss, cnn.losses, cnn.l2_loss, cnn.accuracy],
                feed_dict)
            time_str = datetime.datetime.now().isoformat()
            print("{}: step {}, loss {:g}, entropy_loss {:g}, l2_loss {:g}, acc {:g}".format(time_str, step, loss, entro_loss, l2_loss, accuracy))
